...
const requestLiveOrders = src => fetch(
  ...
  .then(orderSpecs => orderSpecs.map(orderSpec => {
    const {tx_hash, give_asset, give_quantity, get_asset, get_quantity} = orderSpec;
    return {
      id: tx_hash,
      srcAsset: give_asset,
      srcQuantity: give_quantity,
      dstAsset: get_asset,
      dstQuantity: get_quantity,
    };
  }));
