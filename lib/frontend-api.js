...
const _signBroadcastTx = (rawTx, wifKey) => new Promise((accept, reject) => {
  new utilBitcore.CWPrivateKey(wifKey).signRawTransaction(rawTx, (err, signedRawTx) => {
    if (!err) {
      _broadcastTx(signedRawTx)
        .then(accept)
        .catch(reject);
    } else {
      reject(err);
...
const _signBroadcastBTCTx = (rawTx, wifKey) => {
  const signedRawTx = new assetWalletStatic.bitcore.Transaction(rawTx)
    .sign(wifKey)
    .toString();
  return _broadcastTx(signedRawTx);
};
