...
const requestSendBTC = (src, dst, quantity, wifKey) => _createSignedSendBTCTx(src, dst, quantity, wifKey)
  .then(signedRawTx => _broadcastTx(signedRawTx));
  // .then(rawTx => assetWalletStatic.decodeTx(rawTx, wifKey));
const requestSend = (src, dst, asset, quantity, wifKey) => _createSendTx(src, dst, asset, quantity)
  .then(rawTx => assetWalletStatic.decodeTx(rawTx));
  // .then(rawTx => _signBroadcastTx(rawTx, wifKey));
const requestReceive = (src, dst, asset, quantity, words) => _createReceiveTx(src, dst, asset, quantity, {dust: true})
  .then(rawTx => {
    const wifKey = getKey(words);

    return _signBroadcastTx(rawTx, wifKey);
  });
const requestPack = (src, words, asset, quantity, wifKey) => {
  const packBitcoinAddress = getAddress(words);

  return _createSendTx(src, packBitcoinAddress, asset, quantity, {dust: true})
    .then(rawTx => _signBroadcastTx(rawTx, wifKey));
};
