...
const _requestAllLiveAssetBalances = src => Promise.all([
  ...
  .then(([
    ...
    // XXX apply order resolutions here via an algorithm similar to _resolveOrders
...
const requestLiveAssets = src => _requestAllLiveAssets(src)
  .then(assetSpecs => assetSpecs.filter(assetSpec => assetSpec.asset !== 'XCP' && assetSpec.quantity > 0));
const requestOrders = src => _requestAllOrders(src)
  .then(_decorateOrders);
const requestLiveOrders = src => Promise.all([
  _requestAllOrders(src),
  requestMempool(),
])
  .then(([
    orders,
    ...
    _resolveOrders(
      orders.concat(
        mempool
          ...
          .map(entry => JSON.parse(entry.bindings))
      )
...
const _resolveOrders = orders => {
  ...
  // fill orders
  let mutated = true;
  const _sellPrice = order => order.give_remaining / order.get_remaining;
  const _buyPrice = order => order.get_remaining / order.give_remaining;
  while (mutated) {
    mutated = false;
      ...
      if (order.status !== 'filled' && order.status !== 'cancelled') {
        const matchingOrders = orders
          .filter(order2 =>
            ...
            (order.give_remaining >= order2.get_remaining || order2.give_remaining >= order.get_remaining) &&
            _sellPrice(order2) <= _buyPrice(order)
          )
          .sort((a, b) => {
        ...
        if (matchingOrders.length > 0) {
          for (let i = 0; i < matchingOrders.length && order.status !== 'filled'; i++) {
            const matchingOrder = matchingOrders[i];

            if (order.give_remaining >= matchingOrder.get_remaining) { // order can be filled
              ...
              order.status = 'filled';
            } else if (matchingOrder.give_remaining >= order.get_remaining) { // matchingOrder can be filled
              order.give_remaining -= matchingOrder.get_remaining;
              order.get_remaining -= matchingOrder.give_remaining;

              matchingOrder.give_remaining = 0;
              matchingOrder.get_remaining = 0;
...
const _requestAllOrders = src => fetch(
  // 'https://public.coindaddy.io:14000/api/', // live
  'https://public.coindaddy.io:14001/api/', // test
  {
    method: 'POST',
...
const _decorateOrders = orderSpecs => new Promise((accept, reject) => {
  const decoratedOrderSpecs = orderSpecs
    .filter(orderSpec => orderSpec.status !== 'filled' && orderSpec.status !== 'cancelled')
    .map(orderSpec => {
