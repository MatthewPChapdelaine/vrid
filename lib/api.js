...
const _normalizeQuantity = quantity => quantity / 1e8;
...
const requestXCPBalance = src => requestAssetBalance(src, 'XCP');
  ...
  .then(body => {
    if (!body.error) {
      ...
      for (let i = 0; i < assetSpecs.length; i++) {
        ...
        assetSpec.quantity = _normalizeQuantity(assetSpec.quantity);
      }
...
const requestLiveAssetBalances = src => Promise.all([
  ...
  .then(([
    ...
    for (let i = 0; i < balances.length; i++) {
      ...
      assetSpec.quantity += _normalizeQuantity(quantity);
    }
