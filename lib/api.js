...
const _createSignedSendBTCTx = (src, dst, value, wifKey) => _requestListUnspent(src)
  .then(utxos => {
    ...
    let index = 0;
    while (tx.inputAmount < tx.outputAmount && index < utxos.length) {
      tx.from(utxos[index++]);
    }
...
const requestMempool = () => fetch(
  ...
  .then(txids => Promise.all(txids.map(txid =>
    fetch(
      'https://hub.zeovr.io:18333', // test
      {
        method: 'POST',
...
const _requestBlockCount = () => fetch(
  'https://hub.zeovr.io:18333', // test
  {
    method: 'POST',
    headers: (() => {
...
const _requestBlockAtHeight = blockHeight => fetch(
  'https://hub.zeovr.io:18333', // test
  {
    method: 'POST',
    headers: (() => {
  ...
  .then(blockHash => requestBlock(blockHash));
const requestBlock = blockHash => fetch(
  'https://hub.zeovr.io:18333', // test
  {
    method: 'POST',
    headers: (() => {
...
const requestBlocks = () => _requestBlockCount()
  .then(blockCount => {
    const blockHeights = [];
    for (let i = blockCount; i >= (blockCount - 10) && i >= 0; i--) {
      blockHeights.push(i);
    }
    return Promise.all(blockHeights.map(blockHeight => _requestBlockAtHeight(blockHeight)));
  });
const requestTransaction = txid => fetch(
  'https://hub.zeovr.io:18333', // test
  {
    method: 'POST',
    headers: (() => {
  ...
  .then(([
    ...
    const orderAssetDiffs = (() => {
      ...
      const allOrders = orders.concat(mempoolOrders)
        .map(order => order.give_asset !== 'BTC' && order.give_asset !== 'XCP');
...
const requestSendBTC = (src, dst, value, wifKey) => _createSignedSendBTCTx(src, dst, value, wifKey)
  .then(signedRawTx => _broadcastTx(signedRawTx));
const requestPackBTC = (src, dst, value, wifKey) => requestSendBTC(src, dst, value, wifKey);
const requestReceiveBTC = (src, dst, value, wifKey) => requestSendBTC(src, dst, value, wifKey);
