...
const pages = {
  ...
  asset: () => {
    ...
    $('#pack-asset-words-input')[0].value = frontendApi.makeWords();
    ...
    return () => {
      ...
      $('#pack-asset-words-input')[0].value = '';
...
const _boot = () => {
  ...
  const packAsssetsWordInput = $('#pack-asset-words-input')[0];
  packAsssetsWordInput.addEventListener('click', e => {
    packAsssetsWordInput.focus();
    packAsssetsWordInput.select();
  });
  $('#pack-asset-form')[0].addEventListener('submit', e => {
    e.preventDefault();

    const words = $('#pack-asset-words-input')[0].value;
    ...
    if (words && !isNaN(quantity)) {
      ...
      frontendApi.requestPack(src, words, asset, quantity, wifKey)
        .then(txid => {
