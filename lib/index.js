...
const frontendApi = require('./frontend-api.js')({crdsUrl: `CRDS_URL`});
...
const _boot = () => {
  ...
  const _login = privateKey => {
    ...
    Cookies.set('privateKey', privateKey, {expires: 365});
    ...
    return Promise.all([
      ...
      .then(([
        ...
        const _getAssetSrc = (asset, quantity) => {
          ...
          return `\
            <div class=icon>
              <img class=img src="${creaturejs.makeStaticCreature('asset:' + baseAsset)}">
            </div>
            <div class=sub>
              <div class=name>${asset}</div>
              ${quantity > 0 ? `<div class=quantity>${quantity}</div>` : ''}
            </div>
          `;
        };
        const _bindDraggableAsset = (el, asset, dragStart = _nop, dragEnd = _nop) => {
          ...
          el._unbindDraggable = () => {
            ...
            el.removeEventListener('dragstart', _dragStart);
            el.removeEventListener('dragend', _dragEnd);
          };
        };
        const _unbindDraggableAsset = el => {
          if (el._unbindDraggable) {
            el._unbindDraggable();
            el._unbindDraggable = null;
          }
        };
        const _loadInventory = () => {
          const inventoryGrid = $('#inventory-grid')[0];
          while (inventoryGrid.hasChildNodes()) {
          ...
          const assetEls = Object.keys(newBalances)
            .map(asset => {
              ...
              el.innerHTML = _getAssetSrc(asset, quantity);
              el.addEventListener('click', e => {
                _navigate('/assets/' + asset);
              });
              _bindDraggableAsset(el, asset);
              return el;
            });
          for (let i = 0; i < assetEls.length; i++) {
            const assetEl = assetEls[i];
            inventoryGrid.appendChild(assetEl);
          }
        ...
        const _loadEquipment = () => {
          ...
          const equipments = (() => {
            const s = Cookies.get('data');
            const j = s && _jsonParse(s);
            const equipments = j && j.equipment;

            if (Array.isArray(equipments) && equipments.length === numEquipments && equipments.every(e => e === null || typeof e === 'string')) {
              return equipments;
            } else {
          ...
          const _saveEquipment = () => {
            Cookies.set('data', JSON.stringify({equipment: equipments}), {expires: 365});
          };

          const equipmentGrid = $('#equipment-grid')[0];
          while (equipmentGrid.hasChildNodes()) {
            equipmentGrid.removeChild(equipmentGrid.lastChild);
          }
          const assetEls = equipments
            .map((asset, i) => {
              if (!(balances[asset] > 0)) {
                asset = null;
              }
              ...
              el.classList.add('asset', 'empty');
              el.innerHTML = `<div class=placeholder>Drag here</div>`;
              ...
              const _setAsset = asset => {
                equipments[i] = asset;
                _saveEquipment();

                el.classList.remove('empty');
                el.innerHTML = _getAssetSrc(asset, 0);

                _unbindDraggableAsset(el);
                _bindDraggableAsset(el, asset, _nop, () => {
                  equipments[i] = null;
                  _saveEquipment();
          ...
          for (let i = 0; i < assetEls.length; i++) {
            const assetEl = assetEls[i];
            equipmentGrid.appendChild(assetEl);
          }
        };
  ...
  const _load = url => {
    ...
    if (pageMatch === 'login') {
      ...
      $('#mint-asset-form')[0].style.display = (balances[baseAsset + ':mint'] > 0) ? null : 'none';
      $('#price-asset-form')[0].style.display = (balances[baseAsset + ':mint'] > 0) ? null : 'none';
