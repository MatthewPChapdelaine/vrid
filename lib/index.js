...
const _boot = () => {
  ...
  const _load = url => {
    let match = url.match(/^\/id(?:\/(login|assets\/[^\/]+|createAsset|buyAsset|monitor))?$/);
  ...
  [loginUsername, assetUsername, createAssetUsername, buyAssetUsername, monitorUsername].forEach(username => {
  ...
  $('#buy-asset-form')[0].addEventListener('submit', e => {
    ...
    if (asset && !isNaN(quantity)) {
      const address = buyAssetUsername.innerText;
      const {privateKey} = JSON.parse(secretValue.innerText);

      frontendApi.requestCreateBuy(address, asset, privateKey)
        .then(result => {
