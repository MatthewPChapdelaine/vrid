...
const pages = {
  ...
  mempool: () => {
    const statusbarEl = $('.statusbar.mempool')[0];
    statusbarEl.style.display = 'flex';
    const pageEl = $('.page.mempool')[0];
    pageEl.style.display = 'block';
...
const _boot = () => {
  ...
  const _login = words => {
    ...
    const _loadSync = () => {
      ...
      mempoolUsername.innerText = address;
  ...
  const mempoolUsername = $('#mempool-username')[0];
  [btcUsername, assetUsername, createAssetUsername, unpackUsername, orderUsername, createOrderUsername, mempoolUsername].forEach(username => {
  ...
  $('#mempool')[0].addEventListener('click', e => {
    _setPage('mempool');
  });
