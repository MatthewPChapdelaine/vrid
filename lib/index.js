...
const _boot = () => {
  ...
  $('#approve-button')[0].addEventListener('click', e => {
    window.postMessage({ // XXX make this actually perform the send and return the txid
      response: 'accept',
    }, '*');
  });
  ...
  if (words) {
    ...
    if (action) {
      if (action === 'send' && dst && asset && !isNaN(quantity)) {
        document.body.classList.add('iframe');
        _setPage('actionConfirm');
      } else if (action === 'verify' && dst && asset && !isNaN(quantity)) {
