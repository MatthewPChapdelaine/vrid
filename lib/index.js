...
const _boot = () => {
  ...
  [btcUsername, assetUsername, createAssetUsername, unpackUsername, orderUsername, createOrderUsername].forEach(username => {
  ...
  $('#pack-btc-form')[0].addEventListener('submit', e => {
    ...
    if (!isNaN(quantity)) {
      ...
      frontendApi.requestPackBTC(src, dst, quantity, wifKey)
        .then(txid => {
