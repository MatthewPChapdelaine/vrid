...
const _boot = () => {
      ...
      Promise.all([
        ...
        .then(([
          ...
          const _loadAssets = () => {
            ...
            const assetEls = assetSpecs.map(({asset, quantity, description}) => {
  ...
  $('#issue-form')[0].addEventListener('submit', e => {
    ...
    if (!isNaN(quantity)) {
      ...
      frontendApi.requestIssuance(src, asset, description, quantity, wifKey)
        .then(txid => {
  ...
  $('#create-asset-form')[0].addEventListener('submit', e => {
    ...
    if (assetName && !isNaN(assetQuantity)) {
      ...
      frontendApi.requestIssuance(src, assetName, assetDescription, assetQuantity, wifKey)
        .then(txid => {
