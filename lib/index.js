...
const pages = {
  ...
  createBurn: () => {
    const navbarEl = $('.navbar.create-burn')[0];
    navbarEl.style.display = 'flex';
...
const _boot = () => {
  ...
  [assetUsername, createAssetUsername, createBurnUsername].forEach(assetUsername => {
  ...
  $('#btc-balance')[0].addEventListener('click', e => {
    ...
    _setPage('asset'); // XXX make this BTC-specific
  });
  $('#xcp-balance')[0].addEventListener('click', e => {
    const asset = 'XCP';

    const assetAssetname = $('#asset-assetname')[0];
    assetAssetname.innerText = asset; // XXX make this XCP-specific
  ...
  $('#create-asset-form')[0].addEventListener('submit', e => {
    ...
    if (assetName && !isNaN(assetQuantity)) {
      ...
      api.requestIssuance(src, assetName, assetDescription, assetQuantity, assetDivisible, wifKey) // XXX make this actually broadcast
        .then(tx => {
