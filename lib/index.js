...
const pages = {
  ...
  unpackAsset: () => {
    const statusbarEl = $('.statusbar.unpack-asset')[0];
    statusbarEl.style.display = 'flex';
    const pageEl = $('.page.unpack-asset')[0];
    pageEl.style.display = 'block';
...
const _boot = () => {
  ...
  const _login = words => {
    ...
    const _loadSync = () => {
      ...
      unpackAssetUsername.innerText = address;
    ...
    const _loadAsync = () => {
      Promise.all([
        ...
        frontendApi.requestLiveXCPBalance(address),
        ...
        .then(([
          ...
          const _loadAssets = () => {
            ...
            const assetEls = assetSpecs.map(({asset, quantity, description, divisible}) => {
  ...
  let assetDivisible = false;
  const createAssetUsername = $('#create-asset-username')[0];
  const createBurnUsername = $('#create-burn-username')[0];
  const unpackAssetUsername = $('#unpack-asset-username')[0];
  const orderUsername = $('#order-username')[0];
  const createOrderUsername = $('#create-order-username')[0];
  [btcUsername, xcpUsername, assetUsername, createAssetUsername, createBurnUsername, unpackAssetUsername, orderUsername, createOrderUsername].forEach(username => {
  ...
  $('#unpack-asset')[0].addEventListener('click', e => {
    _setPage('unpackAsset');
  });
  ...
  $('#send-btc-form')[0].addEventListener('submit', e => {
    ...
    const quantity = parseInt($('#send-btc-quantity-input')[0].value, 10);

    if (dst && !isNaN(quantity)) {
      const src = createAssetUsername.innerText;
      const wifKey = privateKeyValue.innerText;

      frontendApi.requestSendBTC(src, dst, quantity, wifKey)
        .then(txid => {
  ...
  $('#pack-asset-form')[0].addEventListener('submit', e => {
    ...
    if (!isNaN(quantity)) {
      ...
      frontendApi.requestPack(src, dst, asset, quantity, wifKey)
        .then(txid => {
          console.log('pack', {txid});
        })
        .catch(err => {
          console.warn(err);
        });
  ...
  $('#issue-form')[0].addEventListener('submit', e => {
    ...
    if (!isNaN(quantity)) {
      ...
      const divisible = assetDivisible;
      const wifKey = privateKeyValue.innerText;

      frontendApi.requestIssuance(src, asset, description, quantity, divisible, wifKey)
        .then(txid => {
  ...
  $('#cancel-order-form')[0].addEventListener('submit', e => {
    ...
    console.log('cancel order', {orderId, wifKey});

    frontendApi.requestCancelOrder(orderId, wifKey)
      .then(orderid => {
  ...
  $('#unpack-asset-form')[0].addEventListener('submit', e => {
    e.preventDefault();

    const s = $('#unpack-asset-input')[0].value;
    const j = _jsonParse(s);

    if (j && typeof j === 'object' && typeof j.words === 'string' && typeof j.asset === 'string' && typeof j.quantity === 'number') {
      const {words, asset, quantity} = j;
      const src = frontendApi.getAddress(words);
      const dst = assetUsername.innerText;
      const wifKey = frontendApi.getKey(words);

      frontendApi.requestReceive(src, dst, asset, quantity, wifKey)
        .then(txid => {
          console.log('unpack', {txid});
        })
        .catch(err => {
          console.warn(err);
        });
  ...
  $('#create-order-form')[0].addEventListener('submit', e => {
    ...
    if (srcAsset && !isNaN(srcQuantity) && dstAsset && !isNaN(dstQuantity)) {
      const src = frontendApi.getAddress(words);
      const expiration = 100;
      const wifKey = frontendApi.getKey(words);

      frontendApi.requestCreateOrder(src, srcAsset, srcQuantity, dstAsset, dstQuantity, expiration, wifKey)
        .then(orderid => {
