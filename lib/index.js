...
const _renderCharges = () => {
  ...
  const chargeEls = charges
    ...
    .map(({id, srcAddress, dstAddress, srcAsset, srcQuantity, dstAsset, dstQuantity, hash}) => {
      ...
      el.querySelectorAll('.remove')[0].addEventListener('click', e => {
        ...
        frontendApi.requestCreateChargeback(hash, privateKey)
          .then(result => {
...
const _boot = () => {
  ...
  const _load = url => {
    let match = url.match(/^\/id(?:\/(login|assets\/[^\/]+|createAsset|charges|createCharge|monitor))?$/);
    ...
    } else if (pageMatch === 'charges') {
      _setPage('charges');
    } else if (pageMatch === 'createCharge') {
  ...
  [loginUsername, assetUsername, createAssetUsername, chargesUsername, createChargeUsername, monitorUsername].forEach(username => {
