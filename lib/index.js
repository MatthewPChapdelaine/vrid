...
const _boot = () => {
        ...
        .then(([
          ...
          const _loadAssets = () => {
            ...
            const assetEls = Object.keys(balances)
              .map(asset => {
                ...
                el.innerHTML = `\
                  <img class=img src="${creaturejs.makeStaticCreature('asset:' + asset)}">
          ...
          const _loadCharges = () => {
            ...
            const chargeEls = charges.map(({id, srcAsset, srcQuantity, dstAsset, dstQuantity}) => {
              ...
              `;
              el.addEventListener('click', e => {
                $('#charge-id')[0].innerText = id;
                $('#charge-src-asset-name')[0].innerText = srcAsset;
                $('#charge-src-asset-quantity')[0].innerText = srcQuantity;
                $('#charge-dst-asset-name')[0].innerText = dstAsset;
                $('#charge-dst-asset-quantity')[0].innerText = dstQuantity;
  ...
  $('#send-asset-form')[0].addEventListener('submit', e => {
    ...
    if (dstAddress && !isNaN(quantity)) {
      ...
      frontendApi.requestSend(asset, quantity, srcAddress, dstAddress, privateKey)
        .then(result => {
  ...
  $('#issue-form')[0].addEventListener('submit', e => {
    ...
    if (!isNaN(quantity)) {
      ...
      frontendApi.requestIssuance(srcAddress, asset, quantity, privateKey)
        .then(txid => {
  ...
  $('#create-asset-form')[0].addEventListener('submit', e => {
    ...
    if (assetName && !isNaN(assetQuantity)) {
      const srcAddress = createAssetUsername.innerText;
      const {privateKey} = JSON.parse(secretValue.innerText);

      frontendApi.requestIssuance(srcAddress, assetName, assetQuantity, privateKey)
        .then(result => {
