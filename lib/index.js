...
const _boot = () => {
  ...
  const _login = words => {
    ...
    const _loadSync = () => {
      authorizeUsername.innerText = address;
    ...
    const _loadAsync = () => {
      const _requestAuthorizedServers = () => fetch('/id/api/authorizedServers', {
        method: 'GET',
        credentials: 'include',
      })
        .then(res => {
      ...
      Promise.all([
        ...
        .then(([
          ...
          const _normalizeAssetName = asset => asset === 'BTC' ? 'CRD' : asset;
          const _loadOrders = () => {
            const ordersGridEl = $('#orders-grid')[0];
            while (ordersGridEl.hasChildNodes()) {
              ordersGridEl.removeChild(ordersGridEl.lastChild);
            }
            const orderEls = orderSpecs.map(({id, srcAsset, srcQuantity, dstAsset, dstQuantity}) => {
              ...
              el.innerHTML = `\
                <div class=order-side>
                  ...
                  <div class=name>${_normalizeAssetName(srcAsset)}</div>
                </div>
                <div class=order-spacer>&#x21d2;</div>
                <div class=order-side>
                  <div class=quantity>${dstQuantity}</div>
                  <div class=name>${_normalizeAssetName(dstAsset)}</div>
                </div>
              `;
              el.addEventListener('click', e => {
                $('#order-id')[0].innerText = id;
                $('#order-src-asset-name')[0].innerText = _normalizeAssetName(srcAsset);
                $('#order-src-asset-quantity')[0].innerText = srcQuantity;
                $('#order-dst-asset-name')[0].innerText = _normalizeAssetName(dstAsset);
            ...
            for (let i = 0; i < orderEls.length; i++) {
              ...
              ordersGridEl.appendChild(orderEl);
            }
          };
          const _loadAuthorizedServers = () => {
            const authorizedServersEl = $('#authorized-servers')[0];
            const authorizedServersPlaceholderEl = $('#authorized-servers-placeholder')[0];
            while (authorizedServersEl.hasChildNodes()) {
              authorizedServersEl.removeChild(authorizedServersEl.lastChild);
            }
            const authorizedServerEls = authorizedServers.map(authorizedServer => {
              const el = document.createElement('li');
              el.classList.add('authorized-server');
              el.innerHTML = `\
                <div class=url>${authorizedServer}</div>
                <a class=remove>&times;</a>
              `;
              el.querySelector('.remove').addEventListener('click', e => {
                fetch('/id/api/unauthorize', {
                  method: 'POST',
                  headers: (() => {
                  ...
                  .then(() => {
                    authorizedServers.splice(authorizedServers.indexOf(authorizedServer), 1);
                    _loadAuthorizedServers();
                  })
                  .catch(err => {
                    console.warn(err);
                  });
            ...
            for (let i = 0; i < authorizedServerEls.length; i++) {
              const authorizedServerEl = authorizedServerEls[i];
              authorizedServersEl.appendChild(authorizedServerEl);
            }

            authorizedServersPlaceholderEl.style.display = authorizedServers.length > 0 ? 'none' : null;
          };
  ...
  [
    ...
    $('#unpack-login-button')[0],
    $('#order-login-button')[0],
    $('#create-order-login-button')[0],
  ...
  const authorizeUsername = $('#authorize-username')[0];
  ...
  $('#authorize-approve-button')[0].addEventListener('click', e => {
    const url = $('#authorize-url')[0].innerText;

    fetch('/id/api/authorize', {
      method: 'POST',
      headers: (() => {
        const headers = new Headers();
        headers.append('Content-Type', 'application/json');
      ...
      .then(() => {
        console.warn('authorize ok');

        _respond(null, 'accept');
      })
      .catch(err => {
  ...
  $('#authorize-reject-button')[0].addEventListener('click', e => {
  ...
  $('#create-order-form')[0].addEventListener('submit', e => {
    ...
    let srcAsset = $('#create-order-src-asset-input')[0].value;
    ...
    if (srcAsset && (srcAsset.length === 4 || srcAsset === CREDIT_ASSET_NAME) && !isNaN(srcQuantity) && dstAsset && !isNaN(dstQuantity) && !isNaN(expiration)) {
      if (srcAsset === CREDIT_ASSET_NAME) {
        srcAsset = 'BTC';
      }
